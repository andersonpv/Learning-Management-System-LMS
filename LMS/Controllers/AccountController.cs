...
namespace LMS.Controllers
{
    ...
    public class AccountController : CommonController
    {
        ...
        /// <summary>
        /// This is a helper method to get the appropriate uID for a new user. It will find the highest currently
        /// existing uID, add 1 to it, and turn it into a string with the correct number of characters, and begin
        /// with a u at the beginning.
        /// </summary>
        /// <returns>new uID</returns>
        private string GetNewUid()
        {
            string newUid = "u0000001";
            // Get query that gets all of the uIDs from admins, professors, and students.

            var uIDQuery =
                ((
                    from m in db.Students
                    orderby m.UId descending
                    select m.UId
                ).Take(1))
                .Union
                ((
                    from m in db.Administrators
                    orderby m.UId descending
                    select m.UId
                ).Take(1))
                .Union
                ((
                    from m in db.Professors
                    orderby m.UId descending
                    select m.UId
                ).Take(1));

            string max = "u0000000";
            foreach(string id in uIDQuery.ToArray())
            {
                if (max.CompareTo(id) == -1)
                {
                    max = id;
                }
            }
            string idNum = (int.Parse(max.TrimStart('u')) + 1).ToString();

            // Convert that new integer into an 8 char string that begins with u, like u0000000
            newUid = "u" + new string('0', 7 - idNum.Length) + idNum;
